...
export default function AlumnoMaterialesHubPage() {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Seleccione una asignatura para ver recursos y entregas.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {subjects.map((s) => (
          <Link key={s.id} href={`/alumno/materiales/asignatura/${s.id}`}>
            <Card className="h-full transition-colors hover:border-blue-300 hover:bg-blue-50/40">
              ...
              <CardContent>
                <p className="text-sm text-gray-500">
                  {s.code ? `${s.code} · ` : ""}
                  {s.materialsFolderCount}{" "}
                  {s.materialsFolderCount === 1 ? "carpeta" : "carpetas"}
                </p>
              </CardContent>
            </Card>
          </Link>
