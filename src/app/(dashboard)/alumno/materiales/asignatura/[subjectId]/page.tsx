...
export default function AlumnoMaterialesAsignaturaPage() {
  ...
  async function load() {
    const [me, f, subjectsRes] = await Promise.all([
      fetch("/api/auth/me"),
      fetch(`/api/carpetas?subjectId=${encodeURIComponent(subjectId)}`),
    ...
    if (Array.isArray(subjects)) {
      const match = subjects.find((s: { id: string }) => s.id === subjectId);
      if (match) setSubjectName(match.name);
      else if (folderList[0]?.subject?.name) setSubjectName(folderList[0].subject.name);
    } else if (folderList[0]?.subject?.name) {
      setSubjectName(folderList[0].subject.name);
    }

    const entries = await Promise.all(
      (Array.isArray(folderList) ? folderList : []).map(async (folder) => {
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
            ...
            <CardHeader className="pb-2">
              <CardTitle className="flex flex-wrap items-center gap-2 text-base">
                ...
                <span className="text-sm font-normal text-gray-500">
                  {f.group.grade.name} {f.group.name}
                </span>
              </CardTitle>
        ...
        {resourceFolders.length === 0 && (
          <p className="text-sm text-gray-500">No hay carpetas de recursos para esta asignatura.</p>
        )}
      </section>
      ...
      <section className="space-y-3">
          ...
          <Card key={f.id}>
            <CardContent className="space-y-3 py-4">
              ...
              {selectedFolderId === f.id && f.status === "abierta" && (
                <form
                  onSubmit={submit}
                  className="grid gap-3 rounded-md border border-blue-100 bg-blue-50/40 p-3"
                >
        ...
        {submissionFolders.length === 0 && (
          <p className="text-sm text-gray-500">No hay carpetas de entrega para esta asignatura.</p>
        )}
      </section>
    </div>
  );
