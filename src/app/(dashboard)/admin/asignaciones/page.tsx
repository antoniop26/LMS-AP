  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <Button variant={tab === "teacher" ? "default" : "outline"} onClick={() => setTab("teacher")}>Profesores → Asignaturas</Button>
        <Button variant={tab === "student" ? "default" : "outline"} onClick={() => setTab("student")}>Alumnos → Grupos</Button>
      </div>

      {tab === "teacher" && (
        <>
          <Card>
            <CardHeader><CardTitle>Asignar profesor</CardTitle></CardHeader>
            <CardContent>
              <form onSubmit={assignTeacher} className="grid gap-3 sm:grid-cols-4 sm:items-end">
                ...
                <Button type="submit" disabled={!teacherId || !subjectId}>Asignar</Button>
              </form>
            </CardContent>
          </Card>
            ...
            {teacherSubjects.map((row) => (
              <Card key={row.id}>
                <CardContent className="flex items-center justify-between py-3">
                  <p className="text-sm"><span className="font-medium">{row.teacher.fullName}</span> → {row.subject.name}{row.group ? ` · ${row.group.grade.name} ${row.group.name}` : ""}</p>
                  <Button variant="ghost" size="icon" onClick={() => remove("teacher", row.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      {tab === "student" && (
        <>
          <Card>
            <CardHeader><CardTitle>Asignar alumno a grupo</CardTitle></CardHeader>
            <CardContent>
              <form onSubmit={assignStudent} className="grid gap-3 sm:grid-cols-3 sm:items-end">
                ...
                <Button type="submit" disabled={!studentId || !studentGroupId}>Asignar</Button>
              </form>
            </CardContent>
          </Card>
          <div className="grid gap-2">
            {studentGroups.map((row) => (
              <Card key={row.id}>
                <CardContent className="flex items-center justify-between py-3">
                  <p className="text-sm"><span className="font-medium">{row.student.fullName}</span> → {row.group.grade.name} {row.group.name}</p>
                  <Button variant="ghost" size="icon" onClick={() => remove("student", row.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                </CardContent>
              </Card>
            ))}
          </div>
